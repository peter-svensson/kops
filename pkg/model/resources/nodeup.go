...
func (s *NodeUpScript) WithScalewayBootstrap(cloudProvider string) {
	...
	b.WriteString("  NODE_IP=$(ip -4 addr show scope global | grep inet | head -1 | awk \"{print \\$2}\" | cut -d/ -f1); \\\n")
	b.WriteString("  for i in $(seq 1 60); do \\\n")
	b.WriteString("    [ -f \"$KUBELET_CONF\" ] && break; \\\n")
	b.WriteString("    sleep 2; \\\n")
	b.WriteString("  done; \\\n")
	b.WriteString("  if [ -f \"$KUBELET_CONF\" ]; then \\\n")
	b.WriteString("    if [ -f \"$PROVIDER_ID_FILE\" ] && ! grep -q providerID: $KUBELET_CONF; then \\\n")
	...
	b.WriteString("    fi; \\\n")
	b.WriteString("    if [ -n \"$NODE_IP\" ] && ! grep -q \"node-ip\" /etc/systemd/system/kubelet.service; then \\\n")
	b.WriteString("      sed -i \"s|--config=/var/lib/kubelet/kubelet.conf|--node-ip=$NODE_IP --config=/var/lib/kubelet/kubelet.conf|\" /etc/systemd/system/kubelet.service; \\\n")
	b.WriteString("      echo \"Injected --node-ip=$NODE_IP into kubelet service\"; \\\n")
	b.WriteString("    fi; \\\n")
	b.WriteString("    if grep -q \"InstanceGroupRole: ControlPlane\" /opt/kops/conf/kube_env.yaml 2>/dev/null && ! grep -q \"node-labels\" /etc/systemd/system/kubelet.service; then \\\n")
	b.WriteString("      sed -i \"s|--config=/var/lib/kubelet/kubelet.conf|--node-labels=node-role.kubernetes.io/control-plane=,kops.k8s.io/kops-controller-pki= --config=/var/lib/kubelet/kubelet.conf|\" /etc/systemd/system/kubelet.service; \\\n")
	b.WriteString("      echo \"Injected control-plane node labels into kubelet service\"; \\\n")
	b.WriteString("    fi; \\\n")
	b.WriteString("  fi'\n")
